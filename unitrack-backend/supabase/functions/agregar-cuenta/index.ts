...
Deno.serve(async (req: Request) => {
	...
	try {
		...
		if (!tienePermiso) {
			return errorResponse('No tiene permiso para registrar cuentas financieras de la directiva');
		}
		...
		return okResponse(`La cuenta ${cuenta.alias} ha sido registrada exitosamente`);
	} catch (err) {
		console.error(err)
